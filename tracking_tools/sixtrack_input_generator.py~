...
def update_mask(NPART,SEEDRAN,mask_name):
	...
	replacements = {'%NPART':str(NPART), '%SEEDRAN':str(SEEDRAN)}
	for line in infile:
...
def assign_idfor():
	...
	assert os.path.exists('mask/fc.3')
...
#get_mask(mask_file,mask_repo) arguably useful
print 'If assertion error: user must provide with the mask file in the mask folder'
assert os.path.exists('mask/%s.mask' %mask_file)		#check that mask file is in mask directory
print mask_file,' correctly found in the mask folder'

new_mask_file = update_mask(bunch_charge,seed,mask_file) # substitutes bunch_charge and seed in mask
...
#print 'Ready to launch MADx'
##home = launch_madx(new_mask_file)

##print 'Next step: assign idfor = 2 in the fc.3 file'
##assign_idfor()
